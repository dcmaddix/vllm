...
def get_config():
    config_file_path = "/home/ubuntu/triton_exp/config.json"
...
for i in range(3, niter):
    M = 2 ** i
    a = torch.randn((M, K), device=DEVICE, dtype=torch.float16)
    b = torch.randn((128, N, K), device=DEVICE, dtype=torch.float16)
    ...
    if not use_fp_8:
        ...
        triton_ms = triton.testing.do_bench(lambda: matmul(a, b), quantiles=quantiles)
